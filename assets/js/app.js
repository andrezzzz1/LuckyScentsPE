// Variables
let articulosCarrito = JSON.parse(localStorage.getItem("carrito")) || []; // Array para almacenar los productos agregados al carrito
...
document.addEventListener("DOMContentLoaded", () => {
  const contenedor = document.getElementById("perfume-container");

  // Solo agrega el evento si existe el contenedor (index)
  if (contenedor) {
    contenedor.addEventListener("click", agregarAlCarrito);
  }

  // Restaurar carrito en TODAS las páginas
  renderizarCarrito();
  actualizarSubtotal();
...
// Función para guardadar el carrito
...
function renderizarCarrito() {
  ...
  articulosCarrito.forEach((producto) => {
    const detalleSize = producto.size ? ` - ${producto.size}` : "";
    ...
    const itemHTML = `
      <div class="container mb-3">
        <div class="row align-items-center border-bottom py-2">
          ...
          <div class="col-6">
            <h6 class="mb-1 title-product">
              ${producto.nombre}${detalleSize}
            </h6>

            <p class="mb-0 detalles-product">
...
function actualizarContadorCarrito() {
  const totalProductos = articulosCarrito.reduce(
...
function generarPedidoWhatsApp() {
  ...
  articulosCarrito.forEach((producto) => {
    ...
    const sizeProducto = producto.size ? ` - ${producto.size}` : "";
    const categoriaProducto = producto.categoria ? producto.categoria : "";

    mensaje += "+ " + nombreProducto.toUpperCase() + sizeProducto.toUpperCase() + "\n";

    if (categoriaProducto) {
      mensaje += "   Categoría: " + categoriaProducto + "\n";
    }

    mensaje += "   Cant: " + cantidad + " x S/ " + precioUnitario.toFixed(2) + " = S/ " + subtotal.toFixed(2) + "\n";
    mensaje += "   --------------------------\n";
  });

  const total = articulosCarrito.reduce((acc, producto) => {
    const precioUnitario = Number(producto.precio) || 0;
    const cantidad = Number(producto.cantidad) || 1;
